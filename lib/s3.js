...
const prettifyFileName = (file, isCLI) => {
  if (isCLI) {
    return `Etcher CLI for ${getOS(file)} ${getArch(file)} ${getArchString(file)}`;
  } else {
...
const getLinks = async (url, prefix, version, getCliPackages) => {
  ...
  .map((item) => {
    ...
    return {
      release: {
        text: prettifyFileName(key, getCliPackages),
        href: `${url}/${key}`
      },
      os: getOS(key),
      arch: getArch(key)
...
export default async (url, prefix, getCliPackages) => {
  const version = await getLatestVersion(url, prefix)
  const links = await getLinks(url, prefix, version, getCliPackages)
